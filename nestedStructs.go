...
func canSendMessage(mToSend messageToSend) bool {
	if mToSend.recipient.number == 0 && mToSend.sender.number != 0 && mToSend.recipient.name != "" && mToSend.sender.name != "" {
		return true
	} else {
		return false
	}
...
// parameter anatomy
// name Type (other languages it is usually type then name)
